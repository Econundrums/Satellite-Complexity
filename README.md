...
```
The "cat1" cluster is the only category statistically significant amongst the other 4 (the unlisted fourth cluster effects would be captured by the intercept, which isn't significant either). The F-statistic is a bit weaker due to the inclusion of two other insignificant variables, however the standard error and adj. r-squared have been lowered/increased slightly, suggesting that model is a (slightly) better fit than our original log-log model with just log_weight as the only explanatory variable (which also doesn't seem to be very sensitive to the inclusion of the dummy variables when looking at its t-statistic and standard error). Overall, the model suggests that depending on what group a satellite bus fits in with, a 1% increase in the bus's weight will increase its T1 costs by ~0.64% for cluster 1, ~0.063% for cluster 2, and will *decrease* costs by ~0.072% for cluster 3, with the intercept capturing the effects of being in cluster 4 which we assume *as our default* that our satellite bus is. Thus, our "complexity factors" are 0.64, 0.063, and 0.072, respectively.
